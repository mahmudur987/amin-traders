...
const UpdateBagModal = ({ isOpen, onClose, refetch, data }) => {
  const {
    offer,
    _id,
    bagType,
  ...
  const [isOffer, setIsOffer] = useState(offer.isOffer);
  const [lessPrice, setlessPrice] = useState(offer.lessPrice);
      ...
      fetch(url, {
        ...
        .then((imagedata) => {
          ...
          if (imagedata.data) {
            ...
            AxiosBaseURL.post(`/bag/${_id}`, newData)
              .then((data) => {
